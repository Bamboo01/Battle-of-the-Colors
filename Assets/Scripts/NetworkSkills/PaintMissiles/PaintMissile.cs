...
public class PaintMissile : MonoBehaviour, ISkill
{
    float missleCount = 6;
    ...
    {
        StartCoroutine(SpawnMissles(startPoint, data, serverHandler, server));
    }

    IEnumerator SpawnMissles(Transform startPoint, ServerCharacterData data, CSZZServerHandler serverHandler, NetworkRoomManagerScript server)
    {
